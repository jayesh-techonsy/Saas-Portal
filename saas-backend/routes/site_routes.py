...
from db import database
from databases import Database
from settings import settings  # ✅ Centralized env config
...
def create_new_reserved_tenant(latest_index: int):
    ...
    try:
        ...
        print("Running bench commands...")
        commands = [
            f"docker exec {settings.DOCKER_CONTAINER_NAME} bench new-site {new_site} --mariadb-root-password admin --admin-password admin --install-app erpnext",
            f"docker exec {settings.DOCKER_CONTAINER_NAME} bench --site {new_site} set-maintenance-mode on"
        ]
...
async def create_tenant():
    try:
        ...
        ssh.exec_command(f"docker exec {settings.DOCKER_CONTAINER_NAME} bench --site {tenant_site} set-maintenance-mode off")
        ssh.close()
