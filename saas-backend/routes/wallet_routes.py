...
#         # # Step 0: Check if subscription already exists
#         # sub_url = f"{ERP_BASE_URL}/api/resource/My Subscription/{tenant_id}"
#         # sub_check = requests.get(sub_url, headers=headers, verify=False)

#         # if sub_check.status_code == 200:
#         #     sub_data = sub_check.json().get("data", {})
#         #     if sub_data and sub_data.get("status") == "Active":
#         #         raise HTTPException(
#         #             status_code=400,
#         #             detail=f"Tenant already has an active subscription to plan: {sub_data.get('plan_name')}"
#         #         )

#         # Step 1: Fetch plan by plan_name or name
#         params = {
#             "filters": json.dumps([["plan_name", "=", plan_name]]),
#             "fields": json.dumps(["name", "plan_name", "duration_days", "price", "description", "apps"])
#         }
...
#             "idx": last_idx + 1,
#             "amount": -price,
#             "description": f"Subscribed to {plan['plan_name']} plan",
#             "date": datetime.utcnow().isoformat()
#         }
#         txn_response = requests.post(f"{ERP_BASE_URL}/api/resource/Wallet Transaction", json=txn_payload, headers=headers, verify=False)
#         if txn_response.status_code != 200:
...
#         # Step 5: Create My Subscription (Only if no active one exists)
#         subscription_payload = {
#             "doctype": "My Subscription",
#             "name": tenant_id,
#             "plan_name": plan["plan_name"],
...
#         create_sub = requests.post(f"{ERP_BASE_URL}/api/resource/My Subscription", json=subscription_payload, headers=headers, verify=False)
#         if create_sub.status_code != 200:
#             raise HTTPException(status_code=create_sub.status_code, detail="Failed to create subscription")

#         # Step 6: Install Apps from plan['apps'] (comma-separated string)
#         apps_field = plan.get("apps", "")
#         app_list = [a.strip() for a in apps_field.split(",") if a.strip()]
...
#         top_up_amount = float(payload.get("amount", 0))
...
# def get_all_subscription_plans():
#     try:
#         url = f'{ERP_BASE_URL}/api/resource/All Subscription Plans?fields=["name","plan_name","duration_days","price", "description"]'
#         headers = {
#             "Authorization": f"token {ERP_API_KEY}:{ERP_API_SECRET}"
#         }
...
#         # Get plan details
#         plan_url = f"{ERP_BASE_URL}/api/resource/All Subscription Plans/{payload['plan_name']}"
#         plan_response = requests.get(plan_url, headers=headers, verify=False)
...
#         check_url = f"{ERP_BASE_URL}/api/resource/My Subscription/{tenant_id}"
#         check_response = requests.get(check_url, headers=headers, verify=False)

#         if check_response.status_code == 200:
#             # Update existing subscription
#             response = requests.put(check_url, json=subscription_payload, headers=headers, verify=False)
#         else:
...
#             raise HTTPException(status_code=response.status_code, detail="Failed to create/update subscription")

#         return {
#             "message": "Subscription successful",
#             "subscription": response.json().get("data")
#         }
